...
public class Arm
{
   ...
   public void directKinematic(){
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        double xa1 = xm1 + 0.5*(dx1);
        double ya1 = yt + 0.5*(dy1);
        double beta1 = Math.atan2(dy1, dx1);
        double alpha1 =  Math.PI - (beta1 + Math.PI/2);
        // elbows positions
        xj1 = xa1 - h1*Math.cos(alpha1);
        yj1 = ya1 - h1*Math.sin(alpha1);
        
        double angle1 = beta1+Math.atan2(h1, l1);
        theta1 = 0-angle1;
        ...
        double xa2 = xm2 - 0.5*(dx2);
        double ya2 = ym2 - 0.5*(dy2);
        double beta2 = Math.atan2(dy2, dx2);
        double alpha2 = Math.PI/2 - (Math.PI - beta2);
        // elbows positions
        xj2 = xa2 - h1*Math.cos(alpha2);
        yj2 = ya2 - h1*Math.sin(alpha2);
        // motor angles for both 1st elbow positions
        double angle2 = Math.PI-(beta2+Math.atan2(h2, l2));
        theta2 = 0-angle2;
